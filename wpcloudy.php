...
/*  Copyright 2013 - 2017  Benjamin DENIS  (email : contact@wpcloudy.com)
...
function wpc_get_my_weather($attr) {
	...
	if ( isset( $_POST['wpc_param'] ) ) {
        ...
        if( $wpcloudy_enable_geolocation == 'yes' && $_COOKIE['wpc-detectGeolocation']=='1' && $wpcloudy_enable_geolocation_custom_field != 'yes' ) { 
        	$myweather_current_url = wp_remote_retrieve_body(wp_remote_get("http://api.openweathermap.org/data/2.5/weather?lat=$wpcloudy_lat&lon=$wpcloudy_lon&mode=json&units=$wpcloudy_unit&APPID=$wpc_advanced_api_key"));
        ...
        elseif( $wpcloudy_enable_geolocation == 'yes' && $_COOKIE['wpc-manualGeolocation']=='1' && $wpcloudy_enable_geolocation_custom_field != 'yes' )  {
        	$myweather_current_url = wp_remote_retrieve_body(wp_remote_get("http://api.openweathermap.org/data/2.5/weather?id=$wpcloudy_select_city_id&mode=json&units=$wpcloudy_unit&APPID=$wpc_advanced_api_key"));
        	$myweather_current = json_decode($myweather_current_url);
        	if (!$myweather_current) {
          		_e('Unable to retrieve weather data','wp-cloudy');
			}
	        ...
	        if ($wpc_advanced_set_disable_cache == '1') {
	          	$myweather_current_url = wp_remote_retrieve_body(wp_remote_get("http://api.openweathermap.org/data/2.5/weather?q=$wpcloudy_city,$wpcloudy_state_name,$wpcloudy_country_code&mode=json&units=$wpcloudy_unit&APPID=$wpc_advanced_api_key"));
	          	$myweather_current = json_decode($myweather_current_url);
	          	if (!$myweather_current) {
	          		_e('Unable to retrieve weather data','wp-cloudy');
				}
		        ...
		        && false === ( $myweather_current = get_transient( 'myweather_current_'.$wpc_id.'_'.$wpc_get_the_ID ) ) )  {            
			        $myweather_current_url = wp_remote_retrieve_body(wp_remote_get("http://api.openweathermap.org/data/2.5/weather?q=$wpcloudy_custom_field_city_value,$wpcloudy_custom_field_country_value&mode=json&units=$wpcloudy_unit&APPID=$wpc_advanced_api_key"));
			    	$myweather_current = json_decode($myweather_current_url);
			    	if ($myweather_current) {
				...
				&& false === ( $myweather_current = get_transient( 'myweather_current_'.$wpc_id.'_'.$wpc_get_the_ID ) ) ) {            
			        $myweather_current_url = wp_remote_retrieve_body(wp_remote_get("http://api.openweathermap.org/data/2.5/weather?lat=$wpcloudy_custom_field_lat_value&lon=$wpcloudy_custom_field_lon_value&mode=json&units=$wpcloudy_unit&APPID=$wpc_advanced_api_key")); 
			    	$myweather_current = json_decode($myweather_current_url);
			        if ($myweather_current) {
						set_transient( 'myweather_current_'.$wpc_id.'_'.$wpc_get_the_ID, $myweather_current, $wpc_advanced_set_cache_time * MINUTE_IN_SECONDS );
			        	$myweather_current = get_transient( 'myweather_current_'.$wpc_id.'_'.$wpc_get_the_ID );
        ...
        if( $wpcloudy_hour_forecast && !$wpcloudy_hour_forecast_nd =='' ) {
          	if( $wpcloudy_enable_geolocation == 'yes' && $_COOKIE['wpc-detectGeolocation']=='1' && $wpcloudy_enable_geolocation_custom_field != 'yes' ) {
            	$myweather_url = wp_remote_retrieve_body(wp_remote_get("http://api.openweathermap.org/data/2.5/forecast/?lat=$wpcloudy_lat&lon=$wpcloudy_lon&mode=json&units=$wpcloudy_unit&APPID=$wpc_advanced_api_key"));
          	...
          	elseif( $wpcloudy_enable_geolocation == 'yes' && $_COOKIE['wpc-manualGeolocation']=='1' && $wpcloudy_enable_geolocation_custom_field != 'yes' )  {
            	$myweather_url = wp_remote_retrieve_body(wp_remote_get("http://api.openweathermap.org/data/2.5/forecast/?id=$wpcloudy_select_city_id&mode=json&units=$wpcloudy_unit&APPID=$wpc_advanced_api_key"));
            	$myweather = json_decode($myweather_url);
            	if (!$myweather) {
          			_e('Unable to retrieve weather data','wp-cloudy');
				}
        		...
        		if ($wpc_advanced_set_disable_cache == '1') {
              		$myweather_url = wp_remote_retrieve_body(wp_remote_get("http://api.openweathermap.org/data/2.5/forecast/?q=$wpcloudy_city,$wpcloudy_state_name,$wpcloudy_country_code&mode=json&units=$wpcloudy_unit&APPID=$wpc_advanced_api_key", array( 'timeout' => 10)));
              		$myweather = json_decode($myweather_url);
              		if (!$myweather) {
	          			_e('Unable to retrieve weather data','wp-cloudy');
					}
	              	...
	              	if( $wpcloudy_enable_geolocation == 'yes' && $wpcloudy_enable_geolocation_custom_field == 'yes' && $wpcloudy_custom_field_city_value !='' && $wpcloudy_custom_field_country_value !='' && false === ( $myweather = (get_transient( 'myweather_'.$wpc_id.'_'.$wpc_get_the_ID ) ) ) )  { 
                		$myweather_url = wp_remote_retrieve_body(wp_remote_get("http://api.openweathermap.org/data/2.5/forecast/?q=$wpcloudy_custom_field_city_value,$wpcloudy_custom_field_country_value&mode=json&units=$wpcloudy_unit&APPID=$wpc_advanced_api_key"));
                		$myweather = json_decode($myweather_url);
	                	if ($myweather) {
	              	...
	              	elseif( $wpcloudy_enable_geolocation == 'yes' && $wpcloudy_enable_geolocation_custom_field == 'yes' && $wpcloudy_custom_field_lat_value !='' && $wpcloudy_custom_field_lon_value !='' && false === ( $myweather = get_transient( 'myweather_'.$wpc_id.'_'.$wpc_get_the_ID ) ) )  {  
	                	$myweather_url = wp_remote_retrieve_body(wp_remote_get("http://api.openweathermap.org/data/2.5/forecast/?lat=$wpcloudy_custom_field_lat_value&lon=$wpcloudy_custom_field_lon_value&mode=json&units=$wpcloudy_unit&APPID=$wpc_advanced_api_key"));
	                	$myweather = json_decode($myweather_url);
		                if ($myweather) {
		                	set_transient( 'myweather_'.$wpc_id.'_'.$wpc_get_the_ID, $myweather, $wpc_advanced_set_cache_time * MINUTE_IN_SECONDS );
		                	$myweather = get_transient( 'myweather_'.$wpc_id.'_'.$wpc_get_the_ID );
	                ...
	                && false === ( $myweather = get_transient( 'myweather_'.$wpc_id ) ) ) {
	                	$myweather_url = wp_remote_retrieve_body(wp_remote_get("http://api.openweathermap.org/data/2.5/forecast/?q=$wpcloudy_city,$wpcloudy_state_name,$wpcloudy_country_code&mode=json&units=$wpcloudy_unit&APPID=$wpc_advanced_api_key"));
	                	$myweather = json_decode($myweather_url);
		                if (!$myweather) {
		                }else {
        ...
        if ($wpcloudy_forecast && !$wpcloudy_forecast_nd == '' ) {  
      		if( $wpcloudy_enable_geolocation == 'yes' && $_COOKIE['wpc-detectGeolocation']=='1' && $wpcloudy_enable_geolocation_custom_field != 'yes' ) {
            	$myweather_sevendays_url = wp_remote_retrieve_body(wp_remote_get("http://api.openweathermap.org/data/2.5/forecast/daily?lat=$wpcloudy_lat&lon=$wpcloudy_lon&mode=json&units=$wpcloudy_unit&APPID=$wpc_advanced_api_key&cnt=16"));
            	$myweather_sevendays = json_decode($myweather_sevendays_url);
            	if (!$myweather_sevendays) {
          			_e('Unable to retrieve weather data','wp-cloudy');
                } 
        	}
        	elseif( $wpcloudy_enable_geolocation == 'yes' && $_COOKIE['wpc-manualGeolocation']=='1' && $wpcloudy_enable_geolocation_custom_field != 'yes' )  {
            	$myweather_sevendays_url = wp_remote_retrieve_body(wp_remote_get("http://api.openweathermap.org/data/2.5/forecast/daily?id=$wpcloudy_select_city_id&mode=json&units=$wpcloudy_unit&APPID=$wpc_advanced_api_key&cnt=16"));
            	$myweather_sevendays = json_decode($myweather_sevendays_url);
            	if (!$myweather_sevendays) {
          			_e('Unable to retrieve weather data','wp-cloudy');
                } 
            	...
            	if ($wpc_advanced_set_disable_cache == '1') {
              		$myweather_sevendays_url = wp_remote_retrieve_body(wp_remote_get("http://api.openweathermap.org/data/2.5/forecast/daily?q=$wpcloudy_city,$wpcloudy_state_name,$wpcloudy_country_code&mode=json&units=$wpcloudy_unit&APPID=$wpc_advanced_api_key&cnt=16", array( 'timeout' => 10)));
              		$myweather_sevendays = json_decode($myweather_sevendays_url);
              		if (!$myweather_sevendays) {
	          			_e('Unable to retrieve weather data','wp-cloudy');
	                } 
	              	...
	              	if( $wpcloudy_enable_geolocation == 'yes' && $wpcloudy_enable_geolocation_custom_field == 'yes' && $wpcloudy_custom_field_city_value !='' && $wpcloudy_custom_field_country_value !='' && false === ( $myweather_sevendays = get_transient( 'myweather_sevendays_'.$wpc_id.'_'.$wpc_get_the_ID ) ) ) {             
	                	$myweather_sevendays_url = wp_remote_retrieve_body(wp_remote_get("http://api.openweathermap.org/data/2.5/forecast/daily?q=$wpcloudy_custom_field_city_value,$wpcloudy_custom_field_country_value&mode=json&units=$wpcloudy_unit&APPID=$wpc_advanced_api_key&cnt=16"));
	                	$myweather_sevendays = json_decode($myweather_sevendays_url);
	                	if ($myweather_sevendays) {
		                	set_transient( 'myweather_sevendays_'.$wpc_id.'_'.$wpc_get_the_ID, $myweather_sevendays, $wpc_advanced_set_cache_time * MINUTE_IN_SECONDS );	                	
	                		$myweather_sevendays = get_transient( 'myweather_sevendays_'.$wpc_id.'_'.$wpc_get_the_ID );
		                }
	              	}
	              	elseif( $wpcloudy_enable_geolocation == 'yes' && $wpcloudy_enable_geolocation_custom_field == 'yes' && $wpcloudy_custom_field_lat_value !='' && $wpcloudy_custom_field_lon_value !='' && false === ( $myweather_sevendays = get_transient( 'myweather_sevendays_'.$wpc_id.'_'.$wpc_get_the_ID ) ) ) {              
	                	$myweather_sevendays_url = wp_remote_retrieve_body(wp_remote_get("http://api.openweathermap.org/data/2.5/forecast/daily?lat=$wpcloudy_custom_field_lat_value&lon=$wpcloudy_custom_field_lon_value&mode=json&units=$wpcloudy_unit&APPID=$wpc_advanced_api_key&cnt=16"));
	                	$myweather_sevendays = json_decode($myweather_sevendays_url);
	                	if ($myweather_sevendays) {
		                	set_transient( 'myweather_sevendays_'.$wpc_id.'_'.$wpc_get_the_ID, $myweather_sevendays, $wpc_advanced_set_cache_time * MINUTE_IN_SECONDS );
		                	$myweather_sevendays = get_transient( 'myweather_sevendays_'.$wpc_id.'_'.$wpc_get_the_ID );
	                ...
	                || ($wpcloudy_enable_geolocation != 'yes' && $wpcloudy_enable_geolocation_custom_field == 'yes')
	                || ($wpcloudy_enable_geolocation == 'yes' && $wpcloudy_enable_geolocation_custom_field != 'yes'))
	                && false === ( $myweather_sevendays = get_transient( 'myweather_sevendays_'.$wpc_id ) ) ) {  
	                	$myweather_sevendays_url = wp_remote_retrieve_body(wp_remote_get("http://api.openweathermap.org/data/2.5/forecast/daily?q=$wpcloudy_city,$wpcloudy_state_name,$wpcloudy_country_code&mode=json&units=$wpcloudy_unit&APPID=$wpc_advanced_api_key&cnt=16"));
	                    $myweather_sevendays = json_decode($myweather_sevendays_url);
	                	if (!$myweather_sevendays) {

	                	} else {
		                	set_transient( 'myweather_sevendays_'.$wpc_id, $myweather_sevendays, $wpc_advanced_set_cache_time * MINUTE_IN_SECONDS );	                	
		                	$myweather_sevendays = get_transient( 'myweather_sevendays_'.$wpc_id );
	              		}
	              	}
	            	else {
	                	if ($wpcloudy_enable_geolocation == 'yes' && $wpcloudy_enable_geolocation_custom_field == 'yes') {  
		...
		$time_precipitation   					= $myweather_current->precipitation;
		$time_temperature   					= (ceil($myweather_current->main->temp));
		$owm_link         						= '<a href="http://openweathermap.org/city/'.$myweather_current->id.'" target="_blank" title="'.__('Full weather on OpenWeatherMap','wp-cloudy').'">'.__('Full weather','wp-cloudy').'</a>'; 
		...
		if ($wpcloudy_forecast && !$wpcloudy_forecast_nd == '' ) { 
			...
			while ($i<=14) {
				$forecast_day_feed      = strftime('%w', ($myweather_sevendays->list[$i]->dt));
				    ...
				    $forecast_number_[$i]   	= $myweather_sevendays->list[$i]->weather{0}->id;
				    $forecast_rain_[$i]   		= $myweather_sevendays->list[$i]->rain;
				    $forecast_temp_min_[$i]   	= (round($myweather_sevendays->list[$i]->temp->min));
				    $forecast_temp_max_[$i]   	= (round($myweather_sevendays->list[$i]->temp->max));;
				    $i++;
			  	}
			}
			...
			$display_wind = '
			<div class="wind">'. __( 'Wind', 'wp-cloudy' ) .'<span class="wpc-highlight">'. $time_wind_speed_convert .' '.$time_wind_speed_unit.'</span></div>
			';
			$display_humidity = '
	    ...
	    if( $wpcloudy_current_weather =='yes' && ($wpcloudy_temperature_min_max == 'yes' || $wpcloudy_temperature_min_max == '1' )) {
